...
def plot_multiple_foliums(maps, size_x, size_y):
    template = (
        '<iframe srcdoc="{}" style="float:left; width: {}px; height: {}px; display:inline-block; width: 50%; margin: 0'
        ' auto; border: 2px solid black"></iframe>'
    )
    html_map = ""
    for _, m in enumerate(maps):
...
def construct_cmap(colors, data, name="cmap"):
    sub = []
    un = np.unique(data)
    for _, entry in enumerate(colors):
