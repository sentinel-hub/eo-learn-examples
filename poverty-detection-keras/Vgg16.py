...
def vgg16_net(params):
    ...
    # Set up generators
    ###################################
    train_gen = ImageDataGenerator(preprocessing_function=vgg_preproc, **DF["image_data_generator"])
    ...
    callbacks_phase2 = [
        ...
        ModelCheckpoint(ckpt_fpath, monitor="val_categorical_accuracy", save_weights_only=True, save_best_only=False),
        EarlyStopping(min_delta=0.01, patience=5, verbose=1),
        ReduceLROnPlateau(min_delta=0.1, patience=3, verbose=1),
    ]
    ...
    base_model = VGG16(
        weights="imagenet", include_top=False, input_shape=TP["img_size"], pooling="avg"  # Peel off top layer
    ...
    pred = Dense(params["n_classes"], activation=params["output_activation"], name="dense_output")(x)
    ...
    model.compile(
        optimizer=get_optimizer(params["optimizer"], lr=params["lr_phase1"]), loss=params["loss"], metrics=MP["metrics"]
    )

    print("Phase 1, training near-output layer(s)")
    hist = model.fit_generator(
        train_gen.flow_from_directory(directory=op.join(data_dir, "train"), **DF["flow_from_dir"]),
    ...
    model.compile(
        optimizer=get_optimizer(params["optimizer"], params["lr_phase2"]), loss=params["loss"], metrics=MP["metrics"]
    )

    print(f'/nPhase 2, training from layer {params["freeze_cutoff"]} on.')
    test_iter = test_gen.flow_from_directory(
        directory=op.join(data_dir, "test"),
        shuffle=False,  # Helps maintain consistency in testing phase
        **DF["flow_from_dir"],
    )
    ...
    result_dict = dict(loss=np.min(hist.history["val_loss"][check_ind:]), status=STATUS_OK)
