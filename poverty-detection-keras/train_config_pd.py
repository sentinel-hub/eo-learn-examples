...
class ClasswisePerformance(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        y_pred_probs = self.model.predict_generator(self.test_gen, steps=self.gen_steps)
        y_pred = np.argmax(y_pred_probs, axis=1)

        prec, recall, f1, _ = precision_recall_fscore_support(y_true, y_pred, labels=class_labels)
...
model_params = dict(
    ...
    optimizer=[dict(opt_func="rmsprop"), dict(opt_func="sgd", momentum=hp.uniform("momentum", 0.4, 0.9))],
...
data_flow = dict(
    ...
    flow_from_dir=dict(
        target_size=train_params["img_size"][:2],  # Only want width/height here
        color_mode="rgb",
...
)  # Set to visualize augmentations
